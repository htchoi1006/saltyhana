  ...
  useEffect(() => {
    if (mounted) {
      openbankingWinRef.current = window.open(
        `https://testapi.openbanking.or.kr/oauth/2.0/authorize?response_type=code&client_id=${process.env.REACT_APP_ACCOUNT_CLIENT_ID}&scope=login inquiry transfer&state=${process.env.REACT_APP_ACCOUNT_STATE}&auth_type=0&redirect_uri=http://localhost:3000/connecting-account-finish`, //  or /callback
      );
    } else {
      setMounted(true);
    }
